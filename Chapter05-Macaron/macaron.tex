...
\begin{figure}
        \centering
        \includegraphics[width=0.5\textwidth]{full/MacaronScreenshotHi}
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.49\textwidth]{VersionSpace-16-01-29}
...
\begin{figure}[tb]
    \centering  
    \includegraphics[width=0.52\textwidth]{timelines/Example-Process-Diagram4}
...
\begin{figure} [t]
    \centering
    \includegraphics[width=0.48\textwidth]{paths/Path-ByTime}
...
\begin{figure} [t]
    \centering
    \includegraphics[width=0.48\textwidth]{paths/Path-ByComponent2}
...
\begin{figure} [t]
    \centering
    \includegraphics[width=0.48\textwidth]{paths/Path-ByTrack}
...
\subsubsection{\underline{Editing and playback}}
...
%   More general metaphors were used instead of a direct encoding.
   ...
   \begin{figure}
    \centering
    \includegraphics[width=0.48\textwidth, height=1.25in]{realtime/RealTimeFeedback}
...
%    This seemed closely connected with trying to find the best match of the examples: P1 (adjust), P2  (select; imitate, did the visuals follow the vib track? \& already matched), P3 (see which template mimics the heartbeat th emose), P5 (is it the same, see if it matches; he played it several times to confirm; thought examples were answers part way through, but later started to show learning and was encouraged by vis), P6 (Ã¢ÂÂfit the movement of the heartbeatÃ¢ÂÂ),  P7 (none comes close, but one was inverted; this one had two bums, only oneÃ¢ÂÅ ), P8 (feeling whichever one feels most compatible), P4 (at least one felt like animation, but this one didnÃ¢ÂÂtÃ¢ÂÅ 
...
% %    	These 5 techniques varied based on interface and stage in design. participants did TODO CORRELATION    
    ...
    \subsubsection{Indirect example use -- observe how to design}
...
% \subsection{Implications for VT Design Tools}
    ...
    % Prefeel
    \subsubsection{Realtime ``prefeel" then render} % Prefeel and Render}
    ...
    % Wide Walls
    \subsubsection{Tool flexibility}
