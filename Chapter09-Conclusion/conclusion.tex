...
%
%%%%%%%%%%%%%%%%%%
%\section{Summary of Chapter-Specific Findings}
...
In this section, we examine our findings about the \haxd process, including challenges and strategies.
We comment on how it represents an instantiation of a general design process, but needs explicit support for design activities of browsing, sketching, refining, and sharing.
We then look into recommended strategies we found to generalize across the many different devices that can be found for haptic experiences.
After, we comment on strategies for communicating with haptic experiences, developing expressive, cohesive sets of designs.
Finally, we talk about ways to support customization, and why it is so important in \haxd.
...
%%%%%%%%%%%%%%%%%%
% SubSection - Browse/Sketch/Refine/Share
%%%%%%%%%%%%%%%%%%
...
%With haptic design (like other young  mediums) the lack of either available literal representations or an accepted language  translates to a lack of notation, which in turn hinders the use of ``quick-and-dirty'' methods critical for the flexibility required by sketching. 
%For example, we lack a mapping from engineering to perceptual dimensions. Designers must create in the language of the hardware parameters rather than that of perceptual consumption; and worse, this usually involves programming and potentially modifying hardware.
...
\osC{Consistency, also discuss grips, etc. (though should this be in implementation? Nah, this is an enduring problem)}
...
For users:
To interpret haptic signals, people employ a number of conceptual or translational  \textit{schemas}, often combining them.  % frameworks
...
\inlineHeading{Crowdsourcing feedback:} 
