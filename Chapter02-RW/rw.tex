...
While realistic virtual environments for force-feedback haptic feedback are helpful in medical or training applications  \cite{Okamura2009,VanDerMeijden2009}, we focus on applications \osE{that benefit from} an explicit design step.
...
\osE{Designers rapidly generate and evaluate multiple ideas, iteratively exploring a problem space by articulating and manifesting a solution space; ideas then converge to a final developed concept (or set of concepts).}
...
\osE{Though music lends its vocabulary to touch, describing non-temporal properties remain difficult-to-describe.}
...
%%%%%%%%%%%%
\section{Methodology}
\osE{We used a mixed-method approach to the work presented in this dissertation.}
\osE{We combined design with qualitative and quantitative analysis to gather complementary information.}
\osE{The particular blend of methods depended on each project.}
...
We began with \osE{design and} qualitative techniques to gather rich, generative data from design tools\osE{.} %and design processes, and to inform iteration.
\osE{We then iteratively refined our qualitative methods, and built} towards large-scale data collection for our generated theories with \osE{quantitative analysis and} deployed tools.
...
By building usable support tools, we gained direct knowledge of how to work haptic devices.
...
\osE{O}ur first exploratory study (\autoref{ch:hapticinstrument}) \osE{directly followed phenomenology.} %; in later studies, we combine phenomenology with grounded theory.}
