...
\osE{Haptic technology has evolved along two parallel paths: stimulating sensory mechanisms, or mimic realistic environments.}
...
\osE{they let a designer create convincing sensations without directly recreating physical environments.}
...
\osE{For example, touch can be used a substitute for other senses} \cite{Bach-y-Rita1969}, \osE{and the result can be dramatic, \eg, a blind person using a tongue-based display to seeing a rolling ball well enough to bat it as it falls \cite{Bach-y-Rita2003}.}
...
While realistic virtual environments for force-feedback haptic feedback is helpful in medical or training applications  \cite{Okamura2009,VanDerMeijden2009}, we focus on applications \osE{that benefit from} an explicit design step.
...
Low-level parameters like amplitude, frequency, and duration have been linked to emotions\osE{, \eg, with VT icons} \citet{YongjaeYoo2015} \osE{and mid-air ultrasound stimulation}
\citet{Obrist2015}.
...
\osE{Design thinking is a process and approach to solving problems by creatively and iteratively developing solutions.}
Design is increasingly studied in non-haptic contexts, providing both guidelines and inspiration for \haxd.
...
While we present a cohesive look on \haxd process linked to general guidelines for \haxd support tools, previous work has developed authoring tools, supportive software and hardware platforms, and conceptual frameworks to facilitate \haxd.
...
Haptics courses are taught with a variety of foci \osE{todo: pedagogical viewpoints, purposes, priorities?} including perception, control, and design, providing students with an initial repertoire of skills \cite{Okamura2012, Jones2014}.
...
%%%%%%%%%%%%
\section{Methodology}
\osE{In this dissertation, we use a mixed-method approach.}
\osE{We combine design with qualitative and quantitative analysis to gather complementary information.}
\osE{The particular blend of methods depends on each project.}
...
We begin \osE{design and} qualitative techniques to gather rich, generative data from design tools\osE{.} %and design processes, and to inform iteration.
\osE{We then iteratively refine our qualitative methods, and build} towards large-scale data collection for our generated theories with \osE{quantitative analysis and} deployed tools.
...
\osE{
We take a design approach to understanding \haxd.
%Prototypes manifest design ideas \cite{Lim2008}.
By building usable support tools, we gain direct knowledge of how to work haptic devices.
...
\osE{
Qualitative research methods let us study of complex phenomena in flexible, rigorous way.
...
This data helps us capture the experience of \haxd and develop guidelines for iteration on our tools' early implementations.
We draw from two methodologies in our qualitative studies: phenomenology and grounded theory.
}
...
\osE{O}ur first exploratory study (\autoref{ch:hapticinstrument}) \osE{directly follows phenomenology; in later studies, we combine phenomenology with grounded theory.}
...
\osE{As we start to use quantitative data, we also begin building this infrastructure} using an online editor with analytic logs in Macaron (\autoref{ch:macaron}) and examining the potential for crowdsourced feedback with HapTurk (\autoref{ch:hapturk}).
While a valuable goal, large-scale quantitative feedback on \haxd remains outside the scope of this dissertation.

\osC{Todo: Final paragraph for methodology summing this all up?}
