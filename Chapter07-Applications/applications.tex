...
However, we were able to start quickly, as our software architecture was able to use the same low-level rendering platform as Tactile Animation, and we were able to refine designs easily using human-readable, declarative JSON files.}
...
\revFinalAgain{The TPad Phone was accompanied by a mature API which supported sketching for spatial friction profiles: a image could be easily and quickly loaded to create a static profile.
We found it was more difficult to sketch complex interactions that reacted to input velocity or multitouch.
In both cases, refining designs was slow, as the program needed to be recompiled each time.}
...
We looked at \revFinalAgain{five} mark-making interaction techniques\revFinalAgain{:}
\begin{itemize}
