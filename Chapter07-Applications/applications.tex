...
\chapter{Focused Design Projects}
...
As shown in prior work (cite Hasti), and as we will discuss in \autoref{ch:hapticianinterviews}, customization is an important feature for haptic experiences.
...
\osC{TODO: Go through citations in this section, update from the paper.}
...
The conceptual framework of FeelCraft revolves around the FE library introduced in [17?].
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{Mockup for FeelCraft demo system (cite UIST and AH)}
...
In \autoref{sec:applications:feelcraft}, we designed expressive spatial VT Feel Effects using existing infrastructure via a plugin architecture for desktop applications.
In \autoref{sec:applications:feelmessenger}, we look at the expressiveness of existing infrastructure on Android smartphones for customizable VT effects by implementing customizable VT emojis in a chat program, Feel Messenger \cite{Israr2015}.
...
Here we really wanted to push the limit on what was possible with commodity hardware and APIs, and explore how VT effects could fit into current devices.
In this project, I play the role of a haptic UX designer.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   \caption{Implemented Feel Messenger application demo at World Haptics 2015 (cite).}
...
In Sections \ref{sec:applications:roughsketch} to \ref{sec:applications:cuddlebit}, we investigate other modalities in other applications.
...
It uses piezo-actuated mechanical vibration to create a \osC{TODO: need actual name for the effect, and probably to cite original TPad papers}.
...
To implement RoughSketch, we adapted an open-source Android application (todo) and used the TPad Phone API to control friction using two methods:
pre-loaded textures defined by bitmaps, and
code-based envelopes that programmatically adapt friction based on input values or time.
We used a variety of real-world metaphors to inspire our designs, and rapidly iterated on them.
Illustrations of these are available in \autoref{fig:roughsketch:handout}.
...
These findings are outlined in \autoref{fig:roughsketch:poster}.

\osC{TODO: here or in final discussion, put findings like, some styli worked, and others didn't; and that turning off the tactile sensation was important for convincing people that haptics was valuable, especially when it was subtle; maybe reflect on pinch/zoom? }
...
Education literature has established that embodied, physical interaction can improve learning by making abstractions concrete \cite{montessori_advanced_1917,papert1980,piaget_science_1970}.
Meanwhile,
online courses and interactive lesson plans have increased education access and versatility. 
Haptic technology can integrate these benefits, especially with recent developments for affordable DIY hardware (cite hapkit), but requires % but requires both low-cost hardware (recently enabled by low-cost DIY devices) and
accessible software that enables students to creatively explore haptic environments without writing code.
...
\section{CuddleBit Design Tools}
...
This is important for both everyday interactions with robots, and targeted health applications: robot-based therapy can measurably relax people by breathing (Seidfeger).

The Haptic Creature project (Yohanan) explores the role of touch-based interactions with furry, zoomorphic robots.
...
To explore these concepts more thoroughly, we developed the CuddleBits (cite?): simple, affective robot pals built with a rapid prototyping (sketching) ethic.
To control CuddleBit behaviour and inform \haxd support tools in this complex domain, we developed two software design tools: MacaronBit and Voodle.

MacaronBit (Figure XX) is an adaptation of Macaron (\autoref{ch:macaron}) to control 1-DoF CuddleBit using a familiar track-based metaphor.
...
MacaronBit has been deployed in two studies to develop a set of verified CuddleBit behaviours expressing five emotions from Russel's affective grid: TODO.

Voodle (from ``vocal doodling") is a novel sketching interface to easily create 1-DoF behaviours using non-speech voice, in particular, sub-linguistic ideomorphs (cite) like ``Ooooh" and ``Bwooop."
It is inspired by the onomatopoeia descriptions found with our initial exploration (\autoref{ch:hapticinstrument}) and previous work (Watanabe, Seifi?).
Through a series of user studies, we developed a prioritized set of ideomorphs and how they mapped to movements of the CuddleBit.
