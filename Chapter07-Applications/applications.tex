...
We adopt a \revFinalKM{haptician}'s role \revFinalKM{and practice research through design, gaining} first-hand knowledge into \haxd in a more natural design setting than our one-session lab-based evaluations.

\revFinal{
These focused design projects contributed to our inquiry in three ways:
individually informing our in-depth case studies with practical findings about implementation,
implicitly enriching our final conclusions (\autoref{ch:conclusion}) with reflection in our design process,
and collectively suggesting concrete conclusions about the diversity of haptic experiences (\autoref{sec:generalizingdevices}) and conceptually framing haptic designs (\autoref{ch:conclusion:framing}).
}
...
\begin{enumerate}
    ...
    \revFinalKM{FeelCraft showed that we needed a cohesive experience with visual, audio, and haptic feedback all carefully coordinated to be understandable and engaging to users.}
%    \revFinal{FeelCraft informed how designed sensations from Tactile Animation (\autoref{ch:tactileanimation}) could feel when put in context.}
    ...
    \revFinal{With Feel Messenger, we \revFinalKM{found even low-fidelity vibrations could be engaging, but again needed a clear, engaging story, \eg, with colourful visual emoji}.} %pushed the limits of existing hardware to create engaging vibrotactile feedback, and understood how to create effective phone vibrations for HapTurk (\autoref{ch:hapturk}).}
    ...
    \revFinalKM{With RoughSketch, we found haptic feedback could be successfully structured around different metaphors: does friction feedback literally represent how the act of drawing feels (\eg, slippery finger painting) or how the finished product (\eg,  stucco-like spray paint).}
%    \revFinal{With RoughSketch, we learned how to work with tactile sensations for active touch; friction feedback requires users to move their fingers.}
...
%    \revFinal{By designing active-learning lessons, we worked towards a concrete goal within diverse constraints: multiple stakeholders (teachers, students), low-cost device capabilities, and how to fit haptics into a lesson plan.}
	\revFinalKM{We found that haptic learning environments need to be designed around the stimuli and modality used, not simply added to existing lesson plans.}
    ...
    \revFinal{Voodle and MacaronBit \revFinalKM{confirmed the utility of} explicitly supporting \emph{sketching} and \emph{refining} in a suite of tools.}
\end{enumerate}


\noindent
\osE{Most of this chapter (Sections \ref{sec:applications:feelcraft}-\ref{sec:applications:roughsketch} and \ref{sec:applications:cuddlebit}}\revRG{)}  were primarily presented as demos\osE{. As such,} we present \osE{this chapter's work} in a summary format rather than full reproduction\osE{, and exclude prefaces.}
...
As shown in prior work \cite{Seifi2013,Seifi2014}, and as we discuss in \autoref{ch:hapticianinterviews}, customization is an important feature for haptic experiences.
...
\revFinalAgain{In this chapter, we describe the plug-in architecture, envisioned applications, and our implementation for VT grid arrays displaying Feel Effects (FEs) \cite{Israr2014} for a popular video game, Minecraft.}

\revFinalAgain{This project put Tactile Animation (\autoref{ch:tactileanimation}) in context, as we designed for the same VT grid and domain: multimedia entertainment.
We used a variety of metaphors to create our designs, from established effects (heartbeat, rain \cite{Israr2014}) to new ones (explosion and horse galloping), and found only synchronized visual-audio-haptic effects were effective.
We had to develop the software vertically, writing the Minecraft plugin in tandem with the rendering system and final effects.
Iteration was slow without a mature animation tool.
However, we were able to start quickly, as our software architecture was able to use the same low-level rendering platform as Tactile Animation, and we were able to \emph{refine} designs easily using human-readable, declarative JSON files.}
...
\revFinalAgain{Both projects were a chance to practice VT design and contextualize our in-depth design tools.}

\revFinalAgain{With Feel Messenger, we found that even extremely simple VT icons could be engaging.}
As illustrated previously by \autoref{fig:vib:lofidesign} in \autoref{ch:hapturk}, APIs for \revFinalAgain{VT} feedback are currently limited to a series of pulses.
...
\revFinalAgain{However, we had to ensure that the haptic feedback fit into a narrative.}
We found VT icons were effective \revFinalAgain{only} when they had an engaging visual icon to frame the vibration\revFinalAgain{: a cartoon} cat emoji helped the user understand purring vibration was a purr\revFinalAgain{, but a more abstract motor icon was ineffective.}
\revFinalAgain{We also found that each phone had different dynamics and motors, so the set of designs needed to be adapted for each device.}
...
%The critical components of the application are shown in Figure 2.
...
These effects are associated with direct variation in tactile patterns. Previous studies (\eg, \cite{Brewster2004,MacLean2003}) have used variation in amplitude \revRG{and} duration as typical variation. Our library includes:
...
\revFinalAgain{When working with friction feedback, some design activities were more supported than others.}
\revFinalAgain{The TPad Phone was accompanied by a mature API which supported \emph{sketching} for spatial friction profiles: an image could be easily and quickly loaded to create a static profile.
We found it was more difficult to \emph{sketch} complex interactions that reacted to input velocity or multitouch.
In both cases, \emph{refining} designs was slow, as the program needed to be recompiled each time.}
\revFinalAgain{This project also revealed the complexities of creating a design language, that is, a set of principles for a consistent look and feel, when designing for haptics.
We found this in the diverse metaphors we used to frame friction feedback for different mark making techniques.}

We looked at \revFinalAgain{six} mark-making interaction techniques\revFinalAgain{:}
\begin{itemize}
    \item \textit{Paintbrush}, where you feel paint leaving your finger,
    \item \textit{Pen} and \textit{eraser}, based on real-world writing utensils,
...
\revFinalAgain{In this project, we found that haptic experiences were not compelling when simply added to an existing lesson.
Instead, we found that lessons must be designed around intended modalities, and might be most appropriate when used creatively by students, \ie, through active learning.
We also found that low-cost haptic paddles could render discriminable forces, as long as they fit into the lesson. %; however, a single degree of freedom limited interaction metaphors.
Most importantly, when well-designed, haptic lessons might be able to make lessons more engaging and ground more abstract concepts.
}
...
Voodle (\autoref{fig:voodle}), from ``vocal doodling", is a novel \emph{sketching} interface to easily create 1-DoF behaviours using non-speech voice, in particular, ideophones \cite{Dingemanse2012} like ``Ooooh" and ``Bwooop."
...
\section{Takeaways from Focused Design Projects}

\revFinalAgain{
Our focused design projects reinforced our findings from our in-depth studies and expanded our understanding of \haxd.
We consistently found that the narrative framing or chosen metaphors were important to frame haptic experiences, both to facilitate design and to help users interpret intended meaning.
Visual, audio, and haptic feedback need to be tightly coordinated and designed together, necessitating a holistic and vertical view for designs.
Design activities of \emph{sketching}, \emph{refining}, \emph{browsing} and \emph{sharing} are sometimes supported, but help to identify opportunities for new tools and techniques.
Different feedback modalities, and even different devices (\eg, mobile phones), need customized haptic feedback.
When successful, the haptic actuation can be rudimentary or high-fidelity, as long as the entire experience is considered and designed.
These results informally enrich our final takeaways in \autoref{ch:conclusion}, and directly inform our suggestions for handling the diversity of haptic experiences (\autoref{sec:generalizingdevices}) and conceptually framing haptic designs (\autoref{ch:conclusion:framing}) with narrative and metaphor.
}
