...
%Despite its importance \cite{Seifi2014,Seifi2015}, collecting user feedback on perceptual and emotional (i.e., affective) properties  of tactile sensations in small-scale lab studies is undermined by noise due to individual differences (IDs). %can result in large variation in ratings and inconclusive results.
...
%Multidimensional Scaling (MDS) studies are common, inviting participants compare or group vibrations based on perceived similarity~\cite{Hollins1993,van2003distilling,Pasquero2006,Chan2008,Ternes2008}.
%
...
\inlineHeading{(1) Visual proxies} 
...
\inlineHeading{(2)	Low-fidelity vibration proxy:} 
...
As with \linear, we developed \lofi~vibrations iteratively, first with team feedback, then informal interviews (n=6).
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.5\textwidth, height=2in]{VibratorTime}
...
\subsection{Study 1: In-lab Proxy Vibration Validation (G1)} 
...
\inlineHeading{Overview of Results}
...
\end{figure}
 ...
 \inlineHeading{Duration and Pleasantness were translatable}
...
%    The remaining  two were biased (V10, the longest vibration, and V6). \kmC{explain SLC} % what does biased mean in this context? It sounds like you're saying it's okay to dismiss them because of it.
    ...
    \inlineHeading{Speed and Urgency translated better with \lofi}
	...
	\inlineHeading{Roughness had mixed results; best with \linear}
    ...
    \linear\ [4:3], \original\ [4:4], and \lofi\ [4:5]. 
          ...
          \inlineHeading{Energy was most challenging}
...
%    In addition, designing the right energy levels seems challenging for some vibrations given the current phone actuators (e.g., short pulses with high energy, very high energy signals) while it seems more feasible with visual proxies. This suggests that triangulation of different proxies might be a good strategy. In other words, researchers can use low-fi vibrations to get user feedback on speed and urgency and collect user feedback on roughness and energy using the linear (??) proxy.
    ...
    \inlineHeading{Emphasized visualization outperformed direct plot}
...
%    This suggests that \original~was more accurate at displaying some durations, while \linear~was less accurate but effective at all our vibrations.
    ...
    \inlineHeading{V4,V10 difficult, V9 easy to translate}
...
%V7	V8  7-26		4.4
    ...
    \inlineHeading{Proxy modalities are viable for crowdsourcing (G1,G2:feasibility)} % feasibility
   ...
   Our two-step validation was important as a first look at whether ratings shift dramatically; and we saw no indications of bias or overall shift between locally running proxy modalities and remotely deploying them.
    ...
    \inlineHeading{Triangulation (G3:promising directions/proxies)}
...
%	Taken in tandem, this might improve response accuracy.
    ...
    \inlineHeading{Animate visualizations (G3:promising directions)}
    ...
    \inlineHeading{Sound could represent Energy (G3:promising directions)}
...
Thus, while the raw sound files are not directly translatable, a sound proxy definitely has potential. It could, for example, supplement where the \original~\ waveform failed to perform well on any metric (aside from Duration) but a more expressive visual proxy (\linear) performed better.
    ...
    \inlineHeading{Device dependency and need for Energy model for Vibrations (G4:challenges)}
...
\inlineHeading{Response \& data quality for MTurk \lofi\ vibrations (G4:challenges)}
...
\inlineHeading{Automatic translation (G4:challenges)}
...
\osC{TODO: Connect more with other chapter discussions and intros.}
